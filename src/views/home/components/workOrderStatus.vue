  ...
  mounted(){
    ...
    const getOption = ()=> {
        ...
        */
        ...
        let _seriesItem = [],seriesArray = [];

        xAxisData = _resData.map(item=>item.time).filter(function(item, index, arr) {
        ...
        let legendData =  _resData.map(item=>item.statisticsType).filter(function(item, index, arr) {
                //当前元素，在原始数组中的第一个索引==当前索引值，否则返回当前元素
                return arr.indexOf(item, 0) === index;
            });
        ...
        legendData.forEach((legendName,index)=>{
            ...
            let typeData = _resData.filter(item=>item.statisticsType == legendName)
            ...
            _seriesData.push(
                {
                    ...
                    areaStyle: {
                        color: {
                            ...
                            y2: 1,
                            colorStops: [{
                                offset: 0, color: _areaColor[index] // 0% 处的颜色
                            }, {
    ...
    const updateChart =(option)=> {
        ...
        let fn = () => {
            ...
            // 每次向后滚动一个，最后一个从头开始。
            if (option.dataZoom[0].endValue >= xAxisName.length-1 ) {
